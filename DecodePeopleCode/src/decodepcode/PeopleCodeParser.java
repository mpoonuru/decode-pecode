...
public class PeopleCodeParser {
	...
	static Set<String> specialRefs = new HashSet<>();
...
//			container.pos++;
	...
	abstract class StringParser extends ElementParser
	{
		String getString()
		{
			...
			byte bb;
			while (container.hasMore() && (bb = container.get()) != 0)
			{
	...
	class EmbeddedStringParser extends PureStringParser
	{
		...
		void parse() throws IOException
		{
			...
			while (!(container.get() == 0 && container.readAhead() == 0))
			{
				container.pos++;
			}
			container.pos++;
	...
	class CommentParser extends ElementParser
	{
		...
		void parse() throws IOException {
		    ...
		    int comment_length = container.get() & 0xff;
		    comment_length = comment_length + (container.get()& 0xff) * 256;
		    w.write(
	...
	class ReferenceParser extends ElementParser
	{
		...
		@Override
		void parse() throws IOException {
			int b1 = container.get() & 0xff;
			int b2 = container.get() & 0xff;
...
//					w.write("["+ b+"]");
	...
	class NumberParser extends ElementParser
	{
		...
		void parse() throws IOException
		{
		    ...
		    dValue = container.get();
	    	long val = 0, fact = 1;
	    	for (int i = 0; i < num_bytes; i++)
	    	{
	    		val += fact * (container.get() & 0xff);
	    		fact = fact * 256;
	    	}
...
//	    			out_number = "!!PARSE_ERROR!! " + out_number + "!!" + dValue + "!!";
	...
	final ElementParser parserArray[] = { new IdentifierParser((byte) 0), new PureStringParser((byte) 1),
			new SimpleElementParser((byte) 3, ",", NO_SPACE_BEFORE | SPACE_AFTER),
			new SimpleElementParser((byte) 4, "/"), new SimpleElementParser((byte) 5, ".", PUNCTUATION),
			new SimpleElementParser((byte) 6, "="), new SimpleElementParser((byte) 7, ""), // stop parsing?
			new SimpleElementParser((byte) 8, ">="), new SimpleElementParser((byte) 9, ">"),
			new PureStringParser((byte) 10), /* Function | Method | External Datatype | Class name  0x0A*/
			new SimpleElementParser((byte) 11, "(", NO_SPACE_AFTER), new SimpleElementParser((byte) 12, "<="),
			new SimpleElementParser((byte) 13, "<"), new SimpleElementParser((byte) 14, "-"),
			new SimpleElementParser((byte) 15, "*"), new SimpleElementParser((byte) 16, "<>"),
			new NumberParser((byte) 17, 14), new PureStringParser((byte) 18), /* System variable name */
			new SimpleElementParser((byte) 19, "+"), new SimpleElementParser((byte) 20, ")", NO_SPACE_BEFORE), // 0x14
			new SimpleElementParser((byte) 21, ";", SEMICOLON | NEWLINE_AFTER | NO_SPACE_BEFORE), // 0x15
			new EmbeddedStringParser((byte) 22, "\"", "\""), new SimpleElementParser((byte) 24, "And", AND_STYLE),
			new SimpleElementParser((byte) 25, "Else", ELSE_STYLE),
			new SimpleElementParser((byte) 26, "End-If", ENDIF_STYLE), new SimpleElementParser((byte) 27, "Error"),
			new SimpleElementParser((byte) 28, "If", IF_STYLE), // 0x1c
			new SimpleElementParser((byte) 29, "Not"), new SimpleElementParser((byte) 30, "Or", AND_STYLE),
			new SimpleElementParser((byte) 31, "Then", THEN_STYLE), new SimpleElementParser((byte) 32, "Warning"),
			new ReferenceParser((byte) 33), new SimpleElementParser((byte) 35, "|"),
			...
			new SimpleElementParser((byte) 40, "Until", IF_STYLE), new SimpleElementParser((byte) 41, "For", FOR_STYLE),
			new SimpleElementParser((byte) 42, "To"), new SimpleElementParser((byte) 43, "Step"),
			...
			new SimpleElementParser((byte) 51, "Library"), new SimpleElementParser((byte) 53, "As"),
			new SimpleElementParser((byte) 54, "Value"),
			new SimpleElementParser((byte) 55, "End-Function", END_FUNCTION_STYLE),
			new SimpleElementParser((byte) 56, "Return"), new SimpleElementParser((byte) 57, "Returns"),
			new SimpleElementParser((byte) 58, "PeopleCode"), new SimpleElementParser((byte) 59, "Ref"),
			...
			new SimpleElementParser((byte) 62, "When-Other",
					DECREASE_INDENT | NEWLINE_BEFORE | NEWLINE_AFTER | INCREASE_INDENT),
			...
			new ReferenceParser((byte) 74), new SimpleElementParser((byte) 75, "Null"),
			...
			new SimpleElementParser((byte) 79, "", NEWLINE_AFTER), new NumberParser((byte) 80, 18),
			new SimpleElementParser((byte) 81, "PanelGroup"), new SimpleElementParser((byte) 82, ""),
			...
			new CommentParser(
					(byte) 85/*, NEWLINE_AFTER | COMMENT_ON_SAME_LINE | SPACE_BEFORE*/), /* Less-than asterisk comment */
			new SimpleElementParser((byte) 86, "Constant", NEWLINE_BEFORE_SPACE_AFTER),
			new SimpleElementParser((byte) 87, ":", PUNCTUATION), new SimpleElementParser((byte) 88, "import"),
			new SimpleElementParser((byte) 89, "*"), new SimpleElementParser((byte) 90, "class", FUNCTION_STYLE),
			new SimpleElementParser((byte) 91, "end-class", END_FUNCTION_STYLE),
			new SimpleElementParser((byte) 92, "extends"), new SimpleElementParser((byte) 93, "out"),
			...
			new SimpleElementParser((byte) 96, "readonly"), new SimpleElementParser((byte) 97, "private", ELSE_STYLE),
			...
			new SimpleElementParser((byte) 102, "catch"), new SimpleElementParser((byte) 103, "end-try"),
			new SimpleElementParser((byte) 104, "throw"), new SimpleElementParser((byte) 105, "create"),
			...
			new SimpleElementParser((byte) 111, "abstract"), new SimpleElementParser((byte) 112, "interface"),
			new SimpleElementParser((byte) 113, "end-interface"), new SimpleElementParser((byte) 114, "implements"),
			new SimpleElementParser((byte) 115, "protected", ELSE_STYLE),
			new CommentParser((byte) 117, NEWLINE_BEFORE | COMMENT_ON_SAME_LINE), // #If
			new CommentParser((byte) 118), // #Then
			new CommentParser((byte) 120) // #End-If

	};

	Map<Byte, ElementParser> parsers = new HashMap<>();
...
//	logger.setLevel(Level.SEVERE);
	...
	try {
		while (container.pos < container.bytes.length && !endDetected)
		{
			...
			{
				lastPos = posInPPC;
				lastCodes += new Integer( code &0xff).toString() + " ";
				defects++;
			}
			else
			{
...
//						logger.fine("substring "+ lastPos + ".." + posInPPC);
				...
				{
					defects++;
					logger.severe("NOT found '" + generated + "' at pos " + hex(startPos) + " in " + container.getCompositeKey() + ", code = " + (code & 0xff));
					try {
...
{
	int t = PeopleCodeContainer.objectTypeFromString(dir.getName());
	if (t > 0) {
		type = t;
	} else {
